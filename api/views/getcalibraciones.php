...
switch ($method) {
  ...
  //  $sql = "select M.id as idMaquina, M.nombre as nombreMaquina, mc.* FROM maquinaria M INNER JOIN maquina_calibraciones mc ON M.id = mc.idmaquina WHERE M.idempresa=$idempresa" .$WHERE . " ORDER BY M.nombre, mc.orden"; break;
   if ($version == ''){
    $sql = "select M.id as idMaquina, M.nombre as nombreMaquina, mc.*, pc.idusuario FROM maquinaria M INNER JOIN maquina_calibraciones mc ON M.id = mc.idmaquina INNER JOIN permissionCalibracion pc ON mc.id = pc.idcalibracion   WHERE M.idempresa=$idempresa" .$WHERE . " ORDER BY M.nombre, mc.orden"; break;
   }else{
    $sql = "select M.id as idMaquina, M.nombre as nombreMaquina, mc.*, pl.id as idpermiso, pl.idusuario, pl.idcalibracion as idcalibracionpermiso FROM maquinaria M INNER JOIN maquina_calibraciones mc ON M.id = mc.idmaquina INNER JOIN permissionCalibracion pl ON mc.id = pl.idcalibracion  WHERE M.idempresa=$idempresa AND pl.idempresa = $idempresa " .$WHERE . " ORDER BY M.nombre, mc.orden"; break;
    }
}
