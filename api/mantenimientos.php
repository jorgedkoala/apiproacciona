...
switch ($method) {
  case 'GET':
    $sql = "select * from `$table` WHERE idmaquina=$idmaquina ORDER BY fecha ASC"; break;
  case 'PUT':
...
try{
  ...
  $updated=  mysqli_query($conexion,$update_empresa);// or die('{"success":"false","error":"query->'.mysqli_error($conexion).'"}');
  }
  }
  catch (Exception $e) {
  ...
  try{
  if ($method != "GET"){
  $sql_log = "INSERT INTO  logs SET fecha = '" .date("Y-m-d H:i:s"). "', idusuario=".$userId.", tabla= '".$table."', accion= '".$method."', valor= '".mysqli_real_escape_string($conexion,$sql)."', idempresa=".$idempresa.",plataforma='backoffice' ";
