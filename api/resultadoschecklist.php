...
switch ($method) {
  case 'GET':
    $sql = "select *, resultadoschecklist.id as idr, resultadoschecklistcontrol.id as idrc from resultadoschecklist INNER JOIN checklist ON checklist.id = resultadoschecklist.idchecklist INNER JOIN resultadoschecklistcontrol ON resultadoschecklistcontrol.idresultadochecklist = resultadoschecklist.id LEFT OUTER JOIN usuarios on resultadoschecklist.idusuario = usuarios.id WHERE checklist.id=$idchecklist AND resultadoschecklist.fecha >= '$fechainicio' AND resultadoschecklist.fecha <= '$fechafin' ORDER BY idr DESC,resultadoschecklistcontrol.idcontrolchecklist DESC, resultadoschecklist.fecha DESC "; break;
}
//echo $sql;
