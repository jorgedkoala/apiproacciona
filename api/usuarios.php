...
switch ($method) {
  case 'GET':
    $sql = "select * from `$table` WHERE idempresa=$idempresa order by usuario"; break;
  case 'PUT':
...
try{
if ($method != 'GET'){
$update_empresa = "update empresas set updateusers = updateusers +1 WHERE idempresa=$idempresa";
$updated=  mysqli_query($conexion,$update_empresa);
}
}
catch (Exception $e) {
//  echo '{"success":"false","error logging":"',  $e->getMessage(), '"}';
