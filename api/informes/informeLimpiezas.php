...
$cols=[];
$std=["usuario","nombre","fecha_prevista","fecha","desdcripcion","tipo","supervisor","fecha_supervision","supervision","detalles_supervision"];
...
switch ($method) {
  case 'GET':
    ...
    // from ResultadosControl INNER JOIN controles ON controles.id = ResultadosControl.idcontrol LEFT OUTER JOIN usuarios ON ResultadosControl.idusuario = usuarios.id WHERE controles.idempresa=$idempresa AND ResultadosControl.fecha >= '$fechainicio' AND ResultadosControl.fecha <= '$fechafin' ORDER BY ResultadosControl.fecha DESC"; break;
		$sql = "select usr.usuario,nombre,fecha_prevista,fecha,descripcion,tipo,usr2.usuario as supervisor, fecha_supervision,supervision,detalles_supervision from limpieza_realizada lr INNER JOIN usuarios usr ON lr.idusuario = usr.id LEFT JOIN  usuarios usr2 ON lr.idsupervisor = usr2.id WHERE lr.idempresa=$idempresa AND fecha >= '$fechainicio' AND fecha <= '$fechafin' ORDER BY fecha DESC"; 
		$sql2 = "select count(*) as pendientes  FROM limpieza_zona LZ inner join limpieza_elemento LE on LZ.id=LE.idlimpiezazona where LE.fecha <= CURDATE() and LZ.idempresa =$idempresa and LE.periodicidad NOT LIKE '%por uso%'";
		break;
	}
...
if ($method == 'GET') {
	...
	$pendientes=mysqli_query($conexion,$sql2) or die('{"success":"false","error":"query->'.mysqli_error($conexion).'"}');
	while ($reg=mysqli_fetch_array($pendientes))
	{	
		$limpiezasPendientes = $reg["pendientes"];
	}
	$sql3="select le.nombre, le.fecha, le.periodicidad from limpieza_zona LZ inner JOIN limpieza_elemento le ON LZ.id = le.idlimpiezazona WHERE LZ.idempresa = $idempresa ";
	$elementosLimpieza=mysqli_query($conexion,$sql3) or die('{"success":"false","error":"query->'.mysqli_error($conexion).'"}');
	while ($reg=mysqli_fetch_array($elementosLimpieza))
	{	
		$limpiezas[] = $reg;
	}
	if($registros){
		$result = '{"success":"true","columnas":' . json_encode($cols) . ',"valores":' . json_encode($rows) . ',"pendientes":' . $limpiezasPendientes .',"el":' . json_encode($limpiezas) . '}';
	}
	else {
