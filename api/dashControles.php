...
$sqlControles = array(
    "SELECT ctl.nombre, ctl.fecha_ as fecha, ctl.periodicidad2, 'control' as tipo FROM `controles` ctl WHERE (`fecha_` < CURDATE() AND `idempresa` = ".$idempresa." and periodicidad2 NOT LIKE '%por uso%')",
    "SELECT cl.nombrechecklist as nombre, cl.fecha_ as fecha, cl.periodicidad2, 'checklist' as tipo FROM `checklist` cl WHERE (`fecha_` < CURDATE() AND `idempresa` =".$idempresa." and periodicidad2 NOT LIKE '%por uso%')",
    ...
    "SELECT M.nombre as maquina,M.id , mm.*, 'mantenimiento' as tipo FROM maquinaria M inner join maquina_mantenimiento mm on M.id=mm.idmaquina where (mm.fecha < CURDATE() and M.idempresa = ".$idempresa." and mm.periodicidad NOT LIKE '%por uso%')",
    "SELECT M.nombre as maquina,M.id , mm.*, 'calibracion' as tipo FROM maquinaria M inner join maquina_calibraciones mm on M.id=mm.idmaquina where (mm.fecha < CURDATE() and M.idempresa = ".$idempresa." and mm.periodicidad NOT LIKE '%por uso%')"
);
}
